      ...
      <div className="py-6 md:py-10 space-y-4 md:space-y-6">
        ...
        <div className="flex overflow-x-auto overflow-y-hidden scrollbar-hide px-6 md:px-16 space-x-4 md:space-x-6 pb-6">
          {items.map((item) => (
            ...
            >
              <img src={item.thumbnailUrl} alt={item.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100" />
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-4 md:p-6 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                <div className="flex items-center space-x-2 mb-2 scale-75 md:scale-100 origin-left opacity-0 group-hover:opacity-100 transition-all">
                  <button onClick={(e) => { e.stopPropagation(); navigate(`/watch/${item.id}`); }} className="p-2 md:p-3 bg-white rounded-full shadow-lg hover:bg-brand hover:text-white">
                    <Play className="w-4 h-4 fill-black text-black hover:fill-white" />
                  </button>
                  <button onClick={(e) => toggleMyList(item.id, e)} className="p-2 md:p-3 bg-white/10 backdrop-blur-md border border-white/20 rounded-full hover:bg-white/20">
    ...
    <div className="min-h-screen bg-[#050505] text-white">
      ...
      <div className={`${activeFilter === 'home' && !searchQuery ? '-mt-20 md:-mt-40' : 'pt-24 md:pt-32'} relative z-10 pb-20`}>
        {(activeFilter === 'home' && !searchQuery) ? (
          <>
            {renderRow("Featured Releases", allMedia.slice(0, 5))}
            {renderRow("TV Collections", allMedia.filter(m => m.type === 'series'))}
            {renderRow("Cinematic Movies", allMedia.filter(m => m.type === 'movie'))}
          </>
        ) : (
