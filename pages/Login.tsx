...
import { ShieldCheck, ChevronLeft, Lock, AlertTriangle, Cpu, RefreshCw } from 'lucide-react';
...
const Login: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Access process.env directly
    const envPass = process.env.ADMIN_PASSWORD || process.env.ADMIN_PASS || process.env.PASSWORD;
    const effectivePass = envPass || "admin";
    
    if (password === effectivePass) {
      localStorage.setItem('admin_session', 'true');
      navigate('/admin');
    } else {
      setError('Authorization Failed. Encryption Key Mismatch.');
      setPassword('');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 relative">
      ...
      <div className="w-full max-w-md bg-[#0a0a0a]/80 backdrop-blur-2xl border border-white/5 rounded-[3rem] p-10 md:p-14 z-10 shadow-2xl relative overflow-hidden">
        <div className={`absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-brand to-transparent opacity-50 ${isConfigMissing ? 'via-orange-500' : ''}`} />
        
        <div className="text-center mb-10">
          <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 border shadow-2xl transition-all duration-500 ${isConfigMissing ? 'bg-orange-500/10 border-orange-500/20 shadow-orange-500/10' : 'bg-brand/10 border-brand/20 shadow-brand/10'}`}>
            {isSyncing ? <RefreshCw className="w-8 h-8 text-brand animate-spin" /> : (isConfigMissing ? <Cpu className="w-8 h-8 text-orange-500" /> : <Lock className="w-8 h-8 text-brand" />)}
          </div>
        ...
        {isSyncing ? (
          <div className="flex flex-col items-center justify-center py-10 space-y-4">
            ...
            <p className="text-[9px] font-black uppercase tracking-widest text-gray-600">Syncing Production Vault...</p>
          </div>
        ) : (
            ...
            {isConfigMissing && !error && (
              <div className="bg-orange-500/5 border border-orange-500/10 rounded-2xl p-4 space-y-2">
                <div className="flex items-center space-x-2 text-orange-500">
                  ...
                  <span className="text-[9px] font-black uppercase tracking-widest">Environment Notice</span>
                </div>
                <p className="text-[8px] text-gray-500 font-bold uppercase leading-relaxed">
                  No <code className="text-orange-500/80">ADMIN_PASSWORD</code> detected in environment. Initializing via default developer fallback (Pass: "admin").
                </p>
              </div>
            )}

            {error && (
              <div className="text-[10px] text-center font-black uppercase tracking-widest py-4 px-4 rounded-2xl border animate-pulse text-red-500 bg-red-500/5 border-red-500/10">
                {error}
              </div>
            )}
        ...
        <div className="mt-12 pt-8 border-t border-white/5 text-center">
          <p className="text-[9px] text-gray-700 font-bold uppercase tracking-widest leading-relaxed">
            Unauthorized access attempts are logged. <br/> Platform Protected by StreamX Security Core.
          </p>
        </div>
      </div>
    </div>
